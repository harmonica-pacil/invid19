...
1. Create a directory for the project that you want to create (example:
   `invid19`), then open a Command Prompt (cmd) or Terminal in
   that directory.
...
5. Create a new Django project with this template.
   ...
   ```shell
   django-admin startproject --template="https://codeload.github.com/laymonage/django-template-heroku/zip/template" --extension="py,yml,md" --name="Procfile" invid19 .
   ```

   > Note: rename `invid19` with the desired project name. Notice
...
3. Set the code coverage configuration on GitLab.
   ...
   command.
    ...
    ```shell
    coverage run --include="./*" --omit="venv/*,manage.py,invid19/*" manage.py test
    ```
